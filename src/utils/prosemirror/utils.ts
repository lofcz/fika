...
export { rememberTextSelection, forgetTextSelection, clampRangeToDoc, resolveRememberedRange, restoreTextSelection, autoSelectAll, richTextHtmlEqual } from './selection';
export const isList = (node: Node, schema: Schema) => {
  return node.type === schema.nodes.bullet_list || node.type === schema.nodes.ordered_list;
};
export const addMark = (editorView: EditorView, mark: Mark, selection?: {
  from: number;
  to: number;
}) => {
  if (selection) {
...
export const getMarkAttrs = (view: EditorView) => {
  ...
  if (node?.marks?.length) return node.marks;
...
export const getTextAttrsFromDoc = (doc: Node, attrs: Partial<DefaultAttrs> = {}): TextAttrs => {
  const defaultAttrs: DefaultAttrs = {
    ..._defaultAttrs,
    ...attrs
  };
  let marks: readonly Mark[] = [];
  let align = defaultAttrs.align;
  let bulletList = false;
  let orderedList = false;
  let blockquote = false;
  doc.descendants(node => {
    if (node.isText && node.marks.length && !marks.length) marks = node.marks;
    if (node.type.name === 'paragraph' && node.attrs.align) align = node.attrs.align;
