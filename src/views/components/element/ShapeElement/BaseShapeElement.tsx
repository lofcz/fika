...
const BaseShapeElement = memo((props: IBaseShapeElementProps) => {
  ...
  return <div className={cx('base-element-shape')} style={{
    ...
    height: elementInfo.height + 'px'
      ...
      }}><svg overflow='visible' width={elementInfo.width} height={elementInfo.height}><defs>{elementInfo.pattern ? <PatternDefs id={`base-pattern-${elementInfo.id}`} src={elementInfo.pattern} /> : elementInfo.gradient ? <GradientDefs id={`base-gradient-${elementInfo.id}`} type={elementInfo.gradient.type} colors={elementInfo.gradient.colors} rotate={elementInfo.gradient.rotate} /> : null}</defs><g transform={`scale(${elementInfo.width / elementInfo.viewBox[0]}, ${elementInfo.height / elementInfo.viewBox[1]}) translate(0,0) matrix(1,0,0,1,0,0)`}><path vectorEffect='non-scaling-stroke' strokeLinecap='butt' strokeMiterlimit='8' d={elementInfo.path} fill={fill} stroke={outlineColor} strokeWidth={outlineWidth} strokeDasharray={strokeDashArray} /></g></svg><div className={cx('shape-text', text.align)} style={{
