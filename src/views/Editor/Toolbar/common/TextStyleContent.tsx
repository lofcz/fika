...
import styles from './TextStyleContent.module.scss'
...
import { getHandleElement, useHandleElementId, useHandleElementShallow, useToolbarStoreSelect } from './handleElement'
import type { ShapeText, TextAlignVertical } from '@/types/slides'
...
import BoxInsetControl from './BoxInsetControl'
...
import { applyTextBoxStylePatch, readTextBoxStyle, type TextBoxStylePatch } from './textBoxStyle'
...
function defaultShapeText(): ShapeText {
  ...
  return {
    ...
    defaultColor: resolveElementDefaultFontColor(theme.fontColor, {
      fill: el?.type === 'shape' ? el.fill : undefined,
...
const TextStyleContent = memo(function TextStyleContent() {
  const { LL } = useI18nContext()
  const handleElementId = useHandleElementId()
  const textStyle = useHandleElementShallow(el => readTextBoxStyle(el))
  ...
  const updateBox = useCallback((patch: TextBoxStylePatch) => {
    const el = getHandleElement()
    if (!el) return
    const next = applyTextBoxStylePatch(el, patch, defaultShapeText())
    if (!next) return
    if (next.remove) {
      useSlidesStore.getState().removeElementProps({ id: handleElementId, propName: next.remove })
    }
    if (next.props) {
      useSlidesStore.getState().updateElement({ id: handleElementId, props: next.props })
    }
    addHistorySnapshot()
  ...
  const applyColor = useCallback((value: string) => {
    ...
    updateBox({ fill: value })
  }, [colorTarget, updateBox])
  ...
  const updateFixedHeight = useCallback((fixed: boolean) => {
    if (fixed) updateBox({ fixedHeight: true, vAlign })
    else updateBox({ fixedHeight: false })
  }, [updateBox, vAlign])
  ...
  const showVAlign = fixedHeight
  const paragraphExtraSlot = useMemo(() => (
    showVAlign ? (
      <div className={cx('chip-row')}>
        <FormatChip active={vAlign === 'top'} data-tooltip={LL.editor.stylePanel.shared.textAlignTop()} onClick={() => updateBox({ vAlign: 'top' })}>
          <Icon icon="align-vertical-justify-start" />
        </FormatChip>
        <FormatChip active={vAlign === 'middle'} data-tooltip={LL.editor.stylePanel.shared.textAlignMiddle()} onClick={() => updateBox({ vAlign: 'middle' })}>
          <Icon icon="align-vertical-justify-center" />
        </FormatChip>
        <FormatChip active={vAlign === 'bottom'} data-tooltip={LL.editor.stylePanel.shared.textAlignBottom()} onClick={() => updateBox({ vAlign: 'bottom' })}>
          <Icon icon="align-vertical-justify-end" />
        </FormatChip>
      </div>
    ) : null
  ), [LL, showVAlign, vAlign, updateBox])

  if (!textStyle || !inset) return null

  return (
    <div className={cx('text-style-content')}>
      <RichTextBase
        showColor={false}
        presets={presetsSlot}
        color={colorSlot}
      ...
      <PanelSection label={LL.editor.panel.box()}>
        <div className={cx('field')}>
          ...
          <Select
            ...
            value={lineHeight || 1}
            onUpdateValue={value => updateBox({ lineHeight: value as number })}
        ...
        <BoxInsetControl
          ...
          onUpdateValue={value => updateBox({ inset: value })}
        />
        <div className={cx('field')}>
          ...
          <Select
            ...
            onUpdateValue={value => updateBox({ paragraphSpace: value as number })}
...
export default TextStyleContent
