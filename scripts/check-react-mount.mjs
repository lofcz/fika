...
assert(!applySrc.includes('TextSelection.create(doc, 0, doc.content.size)'), 'filled seed must not select the whole doc (invalid on list/math nodes)')
assert(applySrc.includes('alignAllTextblocks'), 'filled seed aligns textblocks in place')
assert(applySrc.includes('TextSelection.near'), 'filled seed restores the caret with a near selection')
